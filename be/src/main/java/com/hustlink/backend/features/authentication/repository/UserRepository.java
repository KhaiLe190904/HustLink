...
public interface UserRepository extends JpaRepository<User, Long> {
  ...
  @Query("SELECT u FROM users u WHERE u.role <> com.hustlink.backend.features.authentication.model.UserRole.ADMIN AND " + "(:role IS NULL OR u.role = :role) AND " + "(:q IS NULL OR LOWER(u.email) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(u.firstName) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(u.lastName) LIKE LOWER(CONCAT('%', :q, '%'))) AND " + "(:status IS NULL OR " + "(:status = 'BANNED' AND u.banned = true) OR " + "(:status = 'SUSPENDED' AND u.banned = false AND u.suspensionExpiresAt IS NOT NULL AND u.suspensionExpiresAt > :now) OR " + "(:status = 'INACTIVE' AND u.banned = false AND (u.suspensionExpiresAt IS NULL OR u.suspensionExpiresAt <= :now) AND u.emailVerified = false) OR " + "(:status = 'ACTIVE' AND u.banned = false AND (u.suspensionExpiresAt IS NULL OR u.suspensionExpiresAt <= :now) AND u.emailVerified = true)) " + "ORDER BY u.id DESC")
